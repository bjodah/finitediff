...
int test_calculate_weights_3(){
    ...
    double * w3 = malloc(sizeof(double)*ldw*(max_deriv+1));
    const double around = 0.0;
    int flag = calculate_weights(w3, ldw, x3, len_g, max_deriv, around);
    assert(flag == 0);
...
void check_x_exp_mx_(
    ...
    for (unsigned order=0; order <= maxord; ++order){
        ...
        const double adiff = fabs(ref_out.first[order] - ref_out.second[order]);
        REQUIRE( adiff < atol );
    }
    free(ref);
    free(out);
...
int test_apply_fd()
{
    ...
    check_x_exp_mx_(2, 0, g1, -4.0, 1.8);

    double g2[3] = {-0.1, 0.0, 0.1};
    check_x_exp_mx_(2, 0, g2, -4.0, 1.8);

    double g3[3] = {0.9, 1.0, 1.1};
    check_x_exp_mx_(2, x, g3, -4.6, 1.8);
    
    double g4[5] = {0.8, 0.9, 1.0, 1.1, 1.2};
    check_x_exp_mx_(3, x, g4, -7.0, 1.8);

    double g5[7] = {0.7, 0.8, 0.9, 1.0, 1.1, 1.2, 1.3};
    check_x_exp_mx_(5, x, g5, -9.0, 1.4);

    double g6[7] = {0.5, 0.98, 0.99, 1.0, 1.2, 1.3, 1.4};
    check_x_exp_mx_(5, x, g6, -9.3, 1.6);

    double g7[7] = {0.5, 0.0118, 1.0120, 1.0122, 1.2, 1.3, 1.4};
    check_x_exp_mx_(5, x, g7, -9.4, 2.3);
}

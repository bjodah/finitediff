...
from finitediff import (
    interpolate_by_finite_diff, derivatives_at_point_by_finite_diff,
    get_weights
)
...
def test_get_weights():
    ...
    d = [get_weights(np.array(xl[i], dtype=np.float64), 0,
    ...
    test_d(d, 3, 1, [-S(1)/60, S(3)/20, -S(3)/4, S(0), S(3)/4, -S(3)/20,
                     S(1)/60])
    test_d(d, 4, 1, [S(1)/280, -S(4)/105, S(1)/5, -S(4)/5, S(0), S(4)/5,
                     -S(1)/5, S(4)/105, -S(1)/280])

    # Second derivative
    test_d(d, 1, 2, [S(1), -S(2), S(1)])
    test_d(d, 2, 2, [-S(1)/12, S(4)/3, -S(5)/2, S(4)/3, -S(1)/12])
    test_d(d, 3, 2, [S(1)/90, -S(3)/20, S(3)/2, -S(49)/18, S(3)/2,
                     -S(3)/20, S(1)/90])
    test_d(d, 4, 2, [-S(1)/560, S(8)/315, -S(1)/5, S(8)/5, -S(205)/72,
                     S(8)/5, -S(1)/5, S(8)/315, -S(1)/560])

    # Third derivative
    test_d(d, 2, 3, [-S(1)/2, S(1), S(0), -S(1), S(1)/2])
    test_d(d, 3, 3, [S(1)/8, -S(1), S(13)/8, S(0), -S(13)/8, S(1), -S(1)/8])
    test_d(d, 4, 3, [-S(7)/240, S(3)/10, -S(169)/120, S(61)/30, S(0),
                     -S(61)/30, S(169)/120, -S(3)/10, S(7)/240])

    # Fourth derivative
    test_d(d, 2, 4, [S(1), -S(4), S(6), -S(4), S(1)])
    test_d(d, 3, 4, [-S(1)/6, S(2), -S(13)/2, S(28)/3, -S(13)/2, S(2),
                     -S(1)/6])
    test_d(d, 4, 4, [S(7)/240, -S(2)/5, S(169)/60, -S(122)/15, S(91)/8,
    ...
    d = [get_weights(np.array(xl[i], dtype=np.float64), 0,
                     maxorder={0: 1, 1: 2, 2: 4, 3: 4}[i]) for i in range(4)]

    # Zeroth derivative
    test_d(d, 0, 0, [S(1)/2, S(1)/2])
    test_d(d, 1, 0, [-S(1)/16, S(9)/16, S(9)/16, -S(1)/16])
    test_d(d, 2, 0, [S(3)/256, -S(25)/256, S(75)/128, S(75)/128,
                     -S(25)/256, S(3)/256])
    test_d(d, 3, 0, [-S(5)/2048, S(49)/2048, -S(245)/2048, S(1225)/2048,
                     S(1225)/2048, -S(245)/2048, S(49)/2048, -S(5)/2048])

    # First derivative
    test_d(d, 0, 1, [-S(1), S(1)])
    test_d(d, 1, 1, [S(1)/24, -S(9)/8, S(9)/8, -S(1)/24])
    test_d(d, 2, 1, [-S(3)/640, S(25)/384, -S(75)/64, S(75)/64, -S(25)/384,
                     S(3)/640])
    test_d(d, 3, 1, [S(5)/7168, -S(49)/5120,  S(245)/3072, S(-1225)/1024,
...
def test_interpolate_by_finite_diff__multiple_ydata():
    ...
    r = interpolate_by_finite_diff(x, y, xout, maxorder=2, ntail=1, nhead=1)
